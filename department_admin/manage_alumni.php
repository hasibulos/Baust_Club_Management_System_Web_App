...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Logic: Dept Admin hole auto tar nijer dept set hobe, Super Admin hole select kora dept hobe
    $dept = ($current_user_role === 'DEPARTMENT_ADMIN') ? $current_user_dept : ($_POST['department'] ?? '');

    $job = $_POST['current_job'] ?? '';
    $email = $_POST['email'] ?? '';
    $linkedin = $_POST['linkedin_url'] ?? '';

    // Final Validation: Dept Admin onno kono dept pathaleo block hobe
    if ($current_user_role === 'DEPARTMENT_ADMIN' && $dept !== $current_user_dept) {
        die("Error: You can only manage alumni from your own department ($current_user_dept).");
    }
...
<body>
    <div class="container">
        ...
        <form method="post" enctype="multipart/form-data" style="background: #f9f9f9; padding: 15px; border-radius: 8px; border: 1px solid #eee;">
            ...
            <?php else: ?>
                <!-- Dept Admin er jonno department fixed ebong readonly -->
                <input type="text" value="<?= htmlspecialchars($current_user_dept) ?>" disabled>
                <input type="hidden" name="department" value="<?= htmlspecialchars($current_user_dept) ?>">
                <p style="font-size: 0.8rem; color: #666; margin: 0;">* You can only add alumni to <strong><?= htmlspecialchars($current_user_dept) ?></strong></p>
            <?php endif; ?>
